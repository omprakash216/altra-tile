    ...
    <div className={`relative overflow-hidden ${toneClasses} ${className}`}>
      <div className={`absolute inset-0 flex items-center justify-center ${padding}`}>
        <img
          src={src}
          alt={alt}
          className={`relative z-10 h-full w-full ${fitClass} drop-shadow-2xl ${imageClassName}`}
          loading={loading}
        />
      </div>
    </div>
...
export function ProductCard({ product }) {
  return (
    <article className="card-lift group flex h-full flex-col overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
      <Link to={`/category/${product.id}`} className="relative block">
        <CardImage
          src={product.image}
          alt={product.title}
          className="aspect-[1.15] bg-slate-100"
          tone="light"
          padding="p-0"
